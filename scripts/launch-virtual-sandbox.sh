...
#   - 只克隆运行时资产（bin/conda/runtime/seed-assets），绝不复制任何真实登录凭证
...
#
# 平台：支持 macOS (Darwin) 与 Linux。Linux 上 SCIENCE_BIN 默认 ~/.local/bin/claude-science。
set -euo pipefail
...
_dd_real="$(realpath "$DATA_DIR" 2>/dev/null || readlink -f "$DATA_DIR" 2>/dev/null || echo "$DATA_DIR")"
_real_real="$(realpath "$REAL_DIR" 2>/dev/null || readlink -f "$REAL_DIR" 2>/dev/null || echo "$REAL_DIR")"
...
if [[ ! -d "$DATA_DIR/bin" ]]; then
  echo "首次初始化沙箱运行时（克隆，只拷运行时、不拷真实登录）…"
  mkdir -p "$DATA_DIR"
...
# —— 沙箱专属钥匙串（仅 macOS；Linux 上 Science 不使用 Keychain）——
if [[ "$OS" == "Darwin" ]]; then
  ...
  echo "（Linux：跳过钥匙串设置，Science 不使用 Keychain）"
fi
