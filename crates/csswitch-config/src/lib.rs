...
pub fn default_sandbox_port() -> u16 {
    8990
}
...
pub fn validate_runtime_ports(proxy_port: u16, sandbox_port: u16) -> Result<(), String> {
...
pub fn migrate_v1_to_v2(_legacy: serde_json::Value) -> Config {
    // v1-to-v2 migration handled by desktop app; CLI starts with v2.
    Config::default()
}
...
pub fn assert_not_symlink(path: &Path) -> io::Result<()> {
...
pub fn load_from(dir: &Path) -> io::Result<Config> {
    ...
    match detect_version(&data)? {
        ...
        VersionKind::Legacy => {
            // v1 config migration: not supported in CLI.
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "检测到旧版 v1 config.json。请先用桌面版 CSSwitch 打开一次完成迁移，或手动删除 ~/.csswitch/config.json 重新配置。",
            ));
        }
        VersionKind::V2 => {
...
fn normalize_active(mut cfg: Config) -> Config {
    for p in cfg.profiles.iter_mut() {
        if csswitch_templates::by_id(&p.template_id).is_none() {
...
fn backfill_relay_models(cfg: &mut Config) -> Vec<String> {
    ...
    for p in cfg.profiles.iter_mut() {
        ...
        let adapter = csswitch_templates::adapter_for(&p.template_id);
        if adapter == "deepseek" || adapter == "qwen" {
            continue; // native 不需要
        }
        if let Some(def) =
            csswitch_templates::by_id(&p.template_id).and_then(|t| t.builtin_models.first())
        {
