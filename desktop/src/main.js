...
function setBusy(on) {
  busy = on;
  [
    els.startProxyBtn, els.stopBtn, els.newBtn,
...
function applyMode(m) {
  ...
  els.startProxyBtn.textContent = "▶ 启动代理";
}
...
async function persistPorts() {
  ...
  try {
    ...
    if (changed) {
      setMsg("端口已保存。改端口会重置正在运行的代理，请重新启动。", "ok");
      await refreshStatus();
    }
  } catch (e) {
...
// 启动代理：起代理进程（不启动沙箱；沙箱由用户在终端用 csswitch science start 单独启动）。
async function startProxyOnly() {
  if (!state.active_id) {
    setMsg("还没有「当前生效」的配置。请先「＋ 新建」或在列表点「设为当前」选一条。", "err");
    return;
  }
  setBusy(true);
  setMsg("启动代理中…");
  try {
    ...
    setMsg((r.msg || "代理已启动") + " (端口 " + (r.port || "") + ")", "ok");
    await refreshStatus();
  } catch (e) {
    setMsg("启动代理失败：" + e, "err");
  } finally {
...
function wire() {
  [
    "startProxyBtn", "stopBtn", "ltProxy", "ltSandbox", "ltUpstream",
  ...
  els.startProxyBtn.addEventListener("click", heroClick);
